        ...
        if i then
            ...
            if is_windows then
                ...
                if at_i then
                    ...
                    if string.byte(line, at_j+1) == backslashValue then at_j = at_j+2 end
                    local _,endpos = line:find("\"", at_j+1)
                    if string.byte(line, endpos-1) == backslashValue then endpos = endpos-1 end
                    ...
                    if not shouldSkipFile then
                        ...
                        args = args:gsub("-D\\\"", "-D\""):gsub("-I\\\"", "-I\""):gsub("\\\"\\\"\\\"", "__3Q_PLACEHOLDER__"):gsub("\\\"\\\"", "\\\"\""):gsub("\\\" ", "\" "):gsub("\\\\", "/"):gsub(",%s*$", ""):gsub("\" ", "\"\n"):gsub("__3Q_PLACEHOLDER__", "\\\"\\\"\""):gsub("\n[^\n]*$", "")
                        local rspfile = io.open(rspfilepath, "w")
                        rspfile:write(args)
                        rspfile:close()
                    end
                    coroutine.yield()
                    table.insert(contentLines, "\t\t\"command\": \"clang++.exe @\\\"" .. EscapePath(rspfilepath) .."\\\"" .. " ".. EscapePath(currentFilename) .."\",\n")
                end
            else
...
function InitializeCurrentGenData()
    ...
    CurrentGenData.ubtPath = "\"" .. CurrentGenData.config.EngineDir .."/Engine/Binaries/DotNET/UnrealBuildTool/"..ubt_exe.."\""
    CurrentGenData.ueBuildBat = "\"" .. CurrentGenData.config.EngineDir .."/Engine/Build/BatchFiles/"..build_script.."\""
...
function Commands.run(opts)
    ...
    if CurrentGenData.target.withEditor then
        ...
        CurrentGenData.target.PlatformName .. "/UnrealEditor" ..  editorSuffix .. exe_ext .. "\""
        ...
        CurrentGenData.target.PlatformName .. "/" .. CurrentGenData.prjName ..  exeSuffix .. exe_ext .. "\""
